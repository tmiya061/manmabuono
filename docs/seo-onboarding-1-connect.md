...
> 疎通確認（読み取りのみ）まで通ったら止めて。**記事の作成・更新はこの STEP ではまだしない。**」

**この STEP のゴール＝疎通確認が通ること。記事はまだ 1 本も書きません。**
記事の書き方・注意事項は STEP2（`seo-onboarding-2-rules.md`）にあります。**書き始める前に必ずそちらを読んでください。**

担当：宮川（環境構築・テーマ側の実装）。
**詰まったとき、特に権限エラーやトークン発行のエラーが出たときは、回避策を探さずに連絡してください。**
権限は用途に合わせて絞ってあるので、エラー＝設定ミスか、やろうとしていることが想定外か、のどちらかです。
...
**繋がった勢いで記事の作成・更新を試さないでください（禁止ではなく、順番の話です）。**
`/blogs/column` は公開中のブログなので、テスト投稿もそのまま本番に出ます。記事にはバージョン履歴が無いため、上書きすると戻せません。
「下書きで作る」「更新前に退避する」「カテゴリタグは 1 つだけ」といったルールが STEP2 にあるので、**そこまで読んでから書き始めてください。**
