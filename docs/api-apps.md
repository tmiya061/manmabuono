...
## 垣谷さんへの引き継ぎ（STEP1 / STEP2 の2枚）

**1枚にまとめない。**接続と記事執筆で必要な情報が違うので、読む単位を分ける。

> **実際の運用（2026-08-17）**：初回は**2枚同時に渡した**。当初は「STEP1 を渡す → 疎通確認の連絡を受けて STEP2 を渡す」順送りを想定していたが、同時に渡しても STEP2 に安全ルール（下書きで作る／更新前に退避／タグは1つだけ）が揃っているので事故のリスクは変わらない、というオーナー判断。
> よって STEP1 側の「完了したら連絡してください」の節は削除済み。**2枚組のドキュメント**として扱う。
